...
ax1.set_ylim([min(min(betax),min(betay))*0.5,max(max(betax),max(betay))*1.05])
...
ax11.set_ylabel(r'$\eta_{x,y}\ (mm)$',fontsize='x-large')
ax12=ax1.twinx()
ax12.yaxis.set_ticks_position('right')
ax12.spines['right'].set_position(('data', 65))
ax12.set_ylim([-20,5])
l3 = ax12.plot(s1,cyy*1000,'k--')
plt1.eSetPlot(ax12)
ax12.set_ylabel(r'$\Delta_{y}\ (m)$',fontsize='x-large')
